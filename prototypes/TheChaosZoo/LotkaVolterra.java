...
public class LotkaVolterra extends JFrame implements MouseListener, KeyListener {
	...
	if (uniformSpeed)
	    for (int p = 0; p < cellTypes * cellTypes; ++p) {
		IntProbDist d = (IntProbDist) pattern.get(p);
		if (d.size() == 0)
		    d.add (p, 1);
	    }
	...
	for (int p = 0; p < pattern.size(); ++p)
	    ((IntProbDist) pattern.get(p)).normalize();
	...
	((IntProbDist) pattern.get (makeCellPairIndex(pc_old,nc_old))).add (makeCellPairIndex(pc_new,nc_new), prob);
    }
	...
	for (int u = 0; u < patternMatchesPerRefresh; ++u)
		...
		if (uniformSpeed || pc != 0) {
		    ...
		    IntProbDist pd = (IntProbDist) pattern.get(cellPairIndex);
